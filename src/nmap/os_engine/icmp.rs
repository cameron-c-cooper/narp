...
impl IeProbes {
    ...
    /** IeProbes must follow immediately after TCP probes in order to work */
    fn new() -> IeProbes {
        let mut rng = thread_rng();
        let x: f64 = rng.gen();
        let y = (u16::max_value() - 1) as f64 * x;
        IeProbes {
            p1: None,
            p2: None,
            icmp_id: y.floor() as u16
        }
    }
    /** MAKE SURE THE OK() RESULT IS USED AND MOVED TO EchoReplyPacket */
    fn send_packet1(&self, ip_addr: Ipv4Addr) -> Result<Vec<u8>, String> {
        ...
        packet.set_identifier(self.icmp_id);
    ...
    fn send_packet2(&self, ip_addr: Ipv4Addr) -> Result<Vec<u8>, String> {
        let protocol = TransportChannelType::Layer3(IpNextHeaderProtocols::Icmp);
        // TODO move this to a match statement and syslog it. this is a daemon, not a user process
        let (mut tx, mut rx) = transport::transport_channel(1024, protocol)
            .expect("Failed to create transport channel");
        ...
        let mut packet = MutableEchoRequestPacket::new(&mut buf)
            .expect("Failed to create ICMP packet");
        packet.set_icmp_type(EchoRequest);
        packet.set_sequence_number(296);
        packet.set_icmp_code(IcmpCode::new(0));
        packet.set_identifier(self.icmp_id+1);
        // payload of 120 bytes of 0x0
        packet.set_payload(&[0u8; 150]);
        ...
        ip_packet.set_ecn(0x00);
        ip_packet.set_identification(self.icmp_id+1);
        ip_packet.set_flags(DF);
        ip_packet.set_ttl(64);
        ip_packet.set_next_level_protocol(Icmp);
        ip_packet.set_source(get_local_ip_addr());
    ...
    fn send_probes(&mut self, addr: Ipv4Addr) {
        self.p1 = Some(self.send_packet1(addr).expect("icmp probe1 failed"));
        self.p2 = Some(self.send_packet2(addr).expect("icmp probe2 failed"))
    }
}
